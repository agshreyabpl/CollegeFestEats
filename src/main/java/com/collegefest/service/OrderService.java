...
public class OrderService {
    ...
    // ---- Singleton access -------------------------------------------
    // The Observer pattern only works if every screen registers with the
    // SAME OrderService. getInstance() guarantees one shared copy.
    ...
    // ── Observer list ──────────────────────────────────────────
    // CopyOnWriteArrayList: the EDT registers observers while the consumer
    // thread iterates to notify - a plain ArrayList could throw
    // ConcurrentModificationException; this list never does.
    ...
    // Daemon thread: keeps consuming while the app lives, but never
    // prevents the JVM from exiting once every window is closed.
    ...
    public Order placeOrder(String studentId, String vendorId, List<String> itemNames) {
        ...
        int eta = new EtaEngine().calculateEta(vendorId, avgPrepTime);
            ...
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    ...
                    try {
                        ...
                        // One bad order must not kill the consumer forever.
                        ex.printStackTrace();
                    }
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
     ...
     * Package-private + static so the rule can be unit-tested without
     * opening a MongoDB connection (the constructor creates DAOs).
     */
    static boolean isValidTransition(String current, String next) {
