...
/**
 * Modal dialog opened from VendorDashboardFrame's "+ Add Menu Item" button.
 *
 * DAY 2: real form with basic validation (required name, numeric price
 * and prep time). This is intentionally simple manual validation — Step 9
 * will swap it for proper Pattern/Matcher regex checks alongside the rest
 * of the app's input validation pass.
 *
 * TODO(Step 6): on submit, call ItemDAO.insertItem() instead of just
 * handing the parsed fields back to the caller via onSubmit.
 */
